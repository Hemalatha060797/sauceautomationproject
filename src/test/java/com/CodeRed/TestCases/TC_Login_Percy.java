...
public class TC_Login_Percy extends BaseClassNew{
	...
	public void userLoginUsernamePwd(String User, String Pass) throws InterruptedException, IOException {
       	...
       	launchUrl();
		logger.info("Url is Opened -userLoginUsernamePwd");
		maxWindow();
		toHold(8000);
		...
		lp.clickLoginButton();
		logger.info("Clicked on Login Button-userLoginUsernamePwd");
		toHold(8000);
	  
		percy.snapshot("Login Page");
		lp.setUsername(User);
		logger.info("Passed Username in UserField-userLoginUsernamePwd");
	    toHold(5000);
	    
	    lp.setPassword(Pass);
		logger.info("Passed Password in PwdField-userLoginUsernamePwd");
	    toHold(5000);
	    ...
	    toHold(8000);
			...
			if (driver.getCurrentUrl().equals(rbn.getString("HomeURL"))) {
				...
				logger.info("Login Test is Passed-userLoginUsernamePwd");
				toHold(8000);
			}
				...
				logger.info("Login Test is Failed-userLoginUsernamePwd");
				toHold(8000);
			}
			...
			lp.userImage();
			logger.info("Clicked on User Profile-userLoginUsernamePwd ");
			Thread.sleep(2000);
			lp.btnLogout();
			logger.info("Clicked on Logout-userLoginUsernamePwd");
		//	eye.close();
	    }	

...
//login 
	...
	@FindBy(xpath = "//button[@class='dropdown-item py-2 pl-2 pr-5 primary font-semi-bold']")
	@CacheLookup
	...
	public void checkAndClickLogin(int intvalue) {
		try {
			...
			WebElement widgetLink = wait.until(ExpectedConditions.elementToBeClickable(By
					.xpath("//a[@class='text-captiliation text-decoration-none pointer pl-2 primary widget-button']")));
			if (widgetLink != null) {

				js = (JavascriptExecutor) ldriver;
				js.executeScript("arguments[0].click()", widgetLink);
				WebElement loginButton = wait
						.until(ExpectedConditions.elementToBeClickable(By.xpath("(//a[text()='Login'])[1]")));
				loginButton.click();
	...
	public void closePopups(int timeoutSeconds) {

		wait = new WebDriverWait(ldriver, Duration.ofSeconds(timeoutSeconds));
		js = (JavascriptExecutor) ldriver;
		
	//	WebElement clsPopup = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//button[@title='Close']")));

		while (true) {

			try {

				WebElement clsPopup1 = wait
						.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//button[@title='Close']")));
				if (clsPopup1.isDisplayed()) {

					js.executeScript("arguments[0].click()", clsPopup1);

				} else {
					System.out.println("All Popups closed");
					break;
				}
	...
	public void clickLogin(int intvalue) {
		...
		js = (JavascriptExecutor) ldriver;
		js.executeScript("arguments[0].click()", logClick);
		//logClick.click();
	}
	...
	public void userImage(int intvalue) {
		...
		WebElement userImage = wait.until(ExpectedConditions
				.visibilityOfElementLocated(By.xpath("//img[@class='dropdown-toggle widget-header__avatar img-fluid text-center ng-star-inserted']")));
